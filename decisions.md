...
- **Options and decisions:**
  - **Starting the audit's tools on Windows:** (a) `shell: true` there; (b) resolve each `.cmd` path explicitly; (c) spawn each tool's JavaScript entry point through `node`. **Decision: (a).** Every command and argument in `AUDIT_CHECKS` is a constant in that file, so there's nothing for a shell to interpolate, and the arguments are relative paths — a repository in a directory with spaces doesn't change them. (b) hard-codes a layout npm can change. (c) needs a name-to-package map for five tools and breaks when a package moves its entry point. POSIX keeps spawning directly.
