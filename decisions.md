...
- **Consequences:**
  ...
  - **Found by re-running on Windows, twice.** The first fix ran the checks but left two faults behind: a command path containing a space was split, and a missing tool still read as an ordinary failure. Neither could have been seen from macOS, and neither was visible in the first Windows report — the second run is what found them.
